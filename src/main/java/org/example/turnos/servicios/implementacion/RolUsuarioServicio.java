...
public class RolUsuarioServicio implements IRolUsuarioServicio {
    ...
    @Override
    public RolUsuarioDTO traerRolUsuarioPorRole(String role) {
        try {
            RolUsuario entity = rolUsuarioRepositorio.findByRole(role)
                    .orElseThrow(() -> new MiExcepcionPersonalizada("RolUsuario no encontrado con role: " + role));
            return toDTO(entity);
        } catch (Exception e) {
            e.printStackTrace();
